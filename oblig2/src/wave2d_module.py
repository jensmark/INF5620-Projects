...
def solver(I, V, f, b, q, Lx, Ly, Nx, Ny, dt, T,
	...
	if version == 'scalar':
		...
		V_a[:,:] = V(xv, yv)
		u = advance(u, u_1, u_2, f_a, b, q_a, dt, x, y, V_a, step1=True)
	...
	for n in It[1:-1]:
		if version == 'scalar':
		    ...
		    f_a[:,:] = f(xv, yv, t[n])  # precompute, size as u
		    u = advance(u, u_1, u_2, f_a, b, q_a, dt, x, y, None)
...
def advance_vectorized(u, u_1, u_2, f_a, b, q_a, dt, x, y, 
						V_a, step1=False):
	dt2 = dt**2 
	dx = x[1] - x[0]
	dy = y[1] - y[0]
	...
	if step1:
		u[1:-1,1:-1] = u_1[1:-1,1:-1]+(dt2/4.0*dx2)*\
			((q_a[2:,1:-1]+q_a[1:-1,1:-1])*(u_1[2:,1:-1]-u_1[1:-1,1:-1])-\
				(q_a[1:-1,1:-1])+q_a[0:-2,1:-1]*(u_1[1:-1,1:-1]-u_1[0:-2,1:-1]))+\
			(dt2/4.0*dy2)*\
			((q_a[1:-1,2:]+q_a[1:-1,1:-1])*(u_1[1:-1,2:]-u_1[1:-1,1:-1])-\
				(q_a[1:-1,1:-1])+q_a[1:-1,0:-2]*(u_1[1:-1,1:-1]-u_1[1:-1,0:-2]))+\
			dt*f_a[1:-1,1:-1]+dt*V_a[1:-1,1:-1]
		
		# Boundary condition du/dn=0
		u[1:-1,0] = u_1[1:-1,0]+(dt2/4.0*dx2)*\
			((q_a[2:,0]+q_a[1:-1,0])*(u_1[2:,0]-u_1[1:-1,0])-\
				(q_a[1:-1,0])+q_a[0:-2,0]*(u_1[1:-1,0]-u_1[0:-2,0]))+\
			(dt2/4.0*dy2)*\
			((q_a[1:-1,1]+q_a[1:-1,0])*(u_1[1:-1,1]-u_1[1:-1,0])-\
				(q_a[1:-1,0])+q_a[2:,0]*(u_1[1:-1,0]-u_1[2:,0]))+\
			dt*f_a[1:-1,0]+dt*V_a[1:-1,0]
			
		u[1:-1,-1] = u_1[1:-1,-1]+(dt2/4.0*dx2)*\
			((q_a[2:,-1]+q_a[1:-1,-1])*(u_1[2:,-1]-u_1[1:-1,-1])-\
				(q_a[1:-1,-1])+q_a[0:-2,-1]*(u_1[1:-1,-1]-u_1[0:-2,-1]))+\
			(dt2/4.0*dy2)*\
			((q_a[1:-1,-2]+q_a[1:-1,-1])*(u_1[1:-1,2]-u_1[1:-1,-1])-\
				(q_a[1:-1,-1])+q_a[2:,-1]*(u_1[1:-1,-1]-u_1[2:,-1]))+\
			dt*f_a[1:-1,-1]+dt*V_a[1:-1,-1]
			
		u[0,1:-1] = u_1[0,1:-1]+(dt2/4.0*dx2)*\
			((q_a[0,2:]+q_a[0,1:-1])*(u_1[0,2:]-u_1[0,1:-1])-\
				(q_a[0,1:-1])+q_a[0,0:-2]*(u_1[0,1:-1]-u_1[0,0:-2]))+\
			(dt2/4.0*dy2)*\
			((q_a[1,1:-1]+q_a[0,1:-1])*(u_1[1,1:-1]-u_1[0,1:-1])-\
				(q_a[0,1:-1])+q_a[0,2:]*(u_1[0,1:-1]-u_1[0,2:]))+\
			dt*f_a[0,1:-1]+dt*V_a[0,1:-1]
			
		u[-1,1:-1] = u_1[-1,1:-1]+(dt2/4.0*dx2)*\
			((q_a[-1,2:]+q_a[-1,1:-1])*(u_1[-1,2:]-u_1[-1,1:-1])-\
				(q_a[-1,1:-1])+q_a[-1,0:-2]*(u_1[-1,1:-1]-u_1[-1,0:-2]))+\
			(dt2/4.0*dy2)*\
			((q_a[-2,1:-1]+q_a[-1,1:-1])*(u_1[-2,1:-1]-u_1[-1,1:-1])-\
				(q_a[-1,1:-1])+q_a[-1,2:]*(u_1[-1,1:-1]-u_1[-1,2:]))+\
			dt*f_a[-1,1:-1]+dt*V_a[-1,1:-1]
	else:
		u[1:-1,1:-1] = (1.0+B)**(-1)*(2.0*u_1[1:-1,1:-1]+u_2[1:-1,1:-1]*(B-1.0)+\
			(dt2/2.0*dx2)*\
				((q_a[2:,1:-1]+q_a[1:-1,1:-1])*(u_1[2:,1:-1]-u_1[1:-1,1:-1])-\
					(q_a[1:-1,1:-1]+q_a[0:-2,1:-1])*(u_1[1:-1,1:-1]-u_1[0:-2,1:-1]))+\
			(dt2/2.0*dy2)*\
		...
		# Boundary condition du/dn=0	
		u[1:-1,0] = (1.0+B)**(-1)*(2.0*u_1[1:-1,0]+u_2[1:-1,0]*(B-1.0)+\
			(dt2/2.0*dx2)*\
				((q_a[2:,0]+q_a[1:-1,0])*(u_1[2:,0]-u_1[1:-1,0])-\
					(q_a[1:-1,0]+q_a[0:-2,0])*(u_1[1:-1,0]-u_1[0:-2,0]))+\
			(dt2/2.0*dy2)*\
				((q_a[2:,1]+q_a[1:-1,0])*(u_1[2:,1]-u_1[1:-1,0])-\
					(q_a[1:-1,0]+q_a[2:,0])*(u_1[1:-1,0]-u_1[2:,0])))+\
			dt*f_a[1:-1,0]	
		
		u[1:-1,-1] = (1.0+B)**(-1)*(2.0*u_1[1:-1,-1]+u_2[1:-1,-1]*(B-1.0)+\
			(dt2/2.0*dx2)*\
				((q_a[2:,-1]+q_a[1:-1,-1])*(u_1[2:,-1]-u_1[1:-1,-1])-\
					(q_a[1:-1,-1]+q_a[0:-2,-1])*(u_1[1:-1,-1]-u_1[0:-2,-1]))+\
			(dt2/2.0*dy2)*\
				((q_a[2:,-2]+q_a[1:-1,-1])*(u_1[2:,-2]-u_1[1:-1,-1])-\
					(q_a[1:-1,-1]+q_a[2:,-1])*(u_1[1:-1,-1]-u_1[2:,-1])))+\
			dt*f_a[1:-1,-1]	
			
		u[0,1:-1] = (1.0+B)**(-1)*(2.0*u_1[0,1:-1]+u_2[0,1:-1]*(B-1.0)+\
			(dt2/2.0*dx2)*\
				((q_a[0,2:]+q_a[0,1:-1])*(u_1[0,2:]-u_1[0,1:-1])-\
					(q_a[0,1:-1]+q_a[0,0:-2])*(u_1[0,1:-1]-u_1[0,0:-2]))+\
			(dt2/2.0*dy2)*\
				((q_a[1,2:]+q_a[0,1:-1])*(u_1[1,2:]-u_1[0,1:-1])-\
					(q_a[0,1:-1]+q_a[0,2:])*(u_1[0,1:-1]-u_1[0,2:])))+\
			dt*f_a[0,1:-1]
